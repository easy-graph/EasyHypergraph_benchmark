...
if __name__ == '__main__':
    ...
    #     if dataset_name not in acadamic_dataset_lst:
    #         eg_test = test(data=dataset, model = eg_model) *100
    #         # dhg_test = test(data=dhg_dataset, model = dhg_model)*100

    #     else:
    #         eg_test = test2(data=dataset, model = eg_model) *100
    #         # dhg_test = test2(data=dhg_dataset, model = dhg_model)*100

    #     eg_avg_test_acc.append(eg_test)
    # dhg_avg_test_acc.append(dhg_test)
    from torch import tensor

    eg_time_lst = tensor(eg_time_lst)
    print("eg_time:", eg_time_lst)
    with open(args.log_output_path, 'a+') as f:
        # f.write(f"eg avg total time: {rep_eg_total_time/rep_time}\n")
        f.write(f'AVG TIME: {float(eg_time_lst.mean()):.3f} ± {float(eg_time_lst.std()):.3f} ')
        # f.write(f"dhg total time: {rep_dhg_total_time/rep_time}\n")
